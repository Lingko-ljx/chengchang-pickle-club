...
Create indexes for bookings by `(sessionId,status)`, `(date,status,createdAt)`, `(phoneHash,createdAt)`, and `(status,terminalAt,personalDataRedactedAt)`; `booking_codes` uses `sha256(normalizedCode)` as the deterministic document ID and stores only the booking ID; audit logs by `(bookingId,createdAt)`; sessions by `(date,startAt)`; Outbox by `(status,nextAttemptAt)`.
